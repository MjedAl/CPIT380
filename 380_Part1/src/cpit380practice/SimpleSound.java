...
/**
 * The <code>SimpleSound</code> class is an implementation of the Java Sound API specifically designed for use with students. http://java.sun.com/products/java-media/sound/index.html
 * <p>
 * This class allows for easy playback, and manipulation of AU, AIFF, and WAV files.
 * <p>
 *
 * Code & ideas for this class related to playing and viewing the sound were borrowed from the Java Sound Demo: http://java.sun.com/products/java-media/sound/ samples/JavaSoundDemo/
 *
...
public class SimpleSound {
    ...
    /**
     * Constant for the default sample size in bits it is usual to have either 8 or 16
     */
    ...
    /**
     * The explorer for this sound, if it exists. If it exists, this becomes the LineListener for output lines in the Playback class.
     *
     * @see Playback
     */
    ...
    /**
     * The name of the file from which this sound was created. Gets updated every time load from file is called.
     *
     * @see #loadFromFile
     */
    ...
    /**
     * Constructs a <code>SimpleSound</code> of the specified length. This sound will simply consist of an empty byte array, and an <code>AudioFileFormat</code> with the following values:
     * <ul>
     ...
     * </ul>
     * Note that no new sound file is created, we only represent the sound with a buffer and the AudioFileFormat. If a file is desired, then the method <code>writeToFile(String filename)
     ...
     *
     * @param newBuffer a byte array representation of the new sound we want this to represent.
     */
     ...
     *
     * @param newAudioFileFormat the new audioFileFormat that describes this sound.
     * @see AudioFileFormat
     */
    ...
    ///////////////////////// methods /////////////////////////////
    /**
     * Creates an <code>AudioInputStream</code> for this sound from the <code>buffer</code> and the <code>audioFileFormat</code>.
     *
     ...
     * @throws SoundException Will throw under every circumstance. This way we can catch the exception in JES.
     */
    ...
    /**
     * Creates an audioInputStream from this sound, and then writes this stream out to the file with the specified name. If no file exists, one is created. If a file already exists, then it is overwritten. This does not check the extension of the fileName passed in to make sure it agrees with the <code>AudioFileFormat.Type</code> of this sound.
     *
     * @param outFileName The name of the file to write this sound to
     * @throws SoundException if any error is encountered while writing to the file.
     */
    ...
    /**
     * Resets the fields of this sound so that it now represents the sound in the specified file. If successful, the fileName ariable is updated such that it is equivalent to <code>inFileName</code>.
     *
     * @param inFileName the path and filename of the sound we want to represent.
     * @throws SoundException if any problem is encountered while reading in from the file.
     */
    public void loadFromFile(String inFileName) throws SoundException, UnsupportedAudioFileException {
    ...
    /**
     * Creates a new Playback thread and starts it. The thread is guarranteed to finish playing the sound as long as the program doesn't exit before it is done. This method does not block, however. So, if you invoke <code>play()</code> multiple times in a row, sounds will simply play on top of eachother - "accidental mixing"
     *
     * @see Playback
     */
    public void play() {
   ...
   *
     ...
     *
     * @param rate a double representing the change in sampleRate (==frameRate) for playing back this sound
     * @param durInFrames a double representing how much of this sound we want to play.
     * @see SimpleSound#playAtRateInRange(float rate, int startFrame, int endFrame, boolean isBlocking)
     ...
     * @param rate a double representing the change in sampleRate (==frameRate) for playing back this sound
     * @param durInFrames a double representing how much of this sound we want to play
     * @see SimpleSound#playAtRateInRange(float range, int startFrame, int endFrame, boolean isBlocking)
     * @throws SoundException if there are problems playing the sound.
     */
     ...
     * false) </code>.
     *
     * @param rate a float representing the change in sampleRate (==frameRate) for playing back this sound
     * @param startFrame an int representing the frame at which we want to begin playing the sound
     * @param endFrame an int representing the frame at which want to stop playing the sound
     * @see SimpleSound#playAtRateInRange(float range, int startFrame, int endFrame, boolean isBlocking)
     * @throws SoundException if there are problems playing the sound.
     */
     ...
     *
     * @param rate The change in the sampleRate (==frameRate) for playing back this sound. The old SampleRate is multiplied by this value. So, if rate = 2, the sound will play twice as fast (half the length), and if rate = .5, the sound will play half as fast (twice the length).
     ...
     * @param isBlocking If true, this method waits until the thread is done playing the sound before returning. If false, it simply starts the thread and then returns.
     ...
     *
     * @param frameNum the index of the frame to access
     * @return an integer representation of the bytes contained within the specified frame
     * @throws SoundException if the frame number is invalid.
     */
     ...
     * @param frameNum the index of the frame to access
     * @return an int representation of the bytes contained in the specified frame.
     * @throws SoundException if the frameNumber is invalid, or the encoding isn't supported.
     */
     ...
     *
     * @return the number of channels (1 for mono, 2 for stereo), or <code>AudioSystem.NOT_SPECIFIED</code>
     ...
     * @param theFrame the byte array that will be copied into this sound's buffer in place of the specified frame.
     * @throws SoundException if the frameNumber is invalid.
     */
     ...
     * @param sample an int representation of the new sample to put in this sound's buffer at the specified frame
     * @throws SoundException if the frameNumber is invalid, or another problem is encountered
     */
